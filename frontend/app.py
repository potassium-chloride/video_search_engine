...
def combo_search(query,k=10): # sum
	start_t = time.time()
	clip_emb = text2vec(query.replace("#",""))
	doc_enc = doc2vec(query)
	...
	weights = 0.6*getCoefsByPrompt_doc(doc_emb)
	...
	distances_list.sort(key = lambda x:(-x['hashtags'],-x['weighted']) )
